...
result <- predict(linear_svm_model,newdata=test,type='response')
...
library(ROCR)
predictions <- predict(linear_svm_model, newdata=test, type="response")
ROCRpred <- prediction(c(predictions), test$Survived)
...
##########Non linear SVM############
svm_model_rbf <- svm(Survived~ ., 
                     data=train, 
                     method="C-classification", 
                     kernel="radial")
## Predicting Test Data
result <- predict(svm_model_rbf,newdata=test,type='response')
result <- factor(result)
result
...
## ROC Curve and calculating the area under the curve(AUC)
predictions <- predict(svm_model_rbf, newdata=test, type="response")
ROCRpred <- prediction(c(predictions), test$Survived)
ROCRperf <- performance(ROCRpred, measure = "tpr", x.measure = "fpr")
plot(ROCRperf, colorize = TRUE, text.adj = c(-0.2,1.7), print.cutoffs.at = seq(0,1,0.1))
...
## Predicting Test Data
unseen_data<-titanic_data[892:nrow(titanic_data),]
unseen_data$Survived<-NULL
result <- predict(svm_model_rbf,newdata=unseen_data,type='response')
result <- factor(result)
result
length(result)
titanic_data$Survived[893:nrow(titanic_data)]<-result
